const csvString = `1984,Reynolds,Ryan,backend,female,34282.22
...
const parseCsvLines = function (csvString) {
    return csvString.split('\n');
}

const parseCsvColumns = function (csvLines) {
    return csvLines.map(function (line) {
        return line.split(',')
    });
}

const generateFriends = (id) => {
    return Array(1 + ~~(10 * Math.random())).fill(0)
        .map(() => (1 + ~~(100 * Math.random())))
        .filter(el => el !== id)
}

const parseCsvLinesAsObjects = function (csvLines) {
    return csvLines.map(function (line, ix) {
        const cols = line.split(',');
        const id = ix + 1;
        
        return {
            id,
            friends: generateFriends(id),
            birthYear: +cols[0],
            firstName: cols[1],
            lastName: cols[2],
            skill: cols[3],
...
const _users = parseCsvLinesAsObjects(parseCsvLines(csvString));
